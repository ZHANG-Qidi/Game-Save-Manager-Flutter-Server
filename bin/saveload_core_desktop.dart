...
Future<void> copyDirectory(Directory source, Directory destination) async {
  try {
    ...
    await for (final entity in source.list()) {
      final newDestination = Directory(destination.path + Platform.pathSeparator + entity.uri.pathSegments.last);
      if (entity is File) {
...
Future<List<File>> listDirectoryFiles(Directory dir) async {
  try {
    if (!await dir.exists()) {
      throw Exception('Directory does not exist: ${dir.path}');
    }
...
Future<List<Directory>> listDirectorySubDirectories(Directory dir) async {
  try {
    if (!await dir.exists()) {
      throw Exception('Directory does not exist: ${dir.path}');
    }
...
Future<List<String>> gameListFunc() async {
  try {
    ...
    final directories = await listDirectorySubDirectories(dir);
    final directoriesString = directories.map((dir) => dir.path).toList();
...
Future<(List<String>, String, String)> profileListFunc(String game) async {
  try {
    ...
    return (directoriesString, folder, file);
  } catch (e) {
...
Future<void> gameNew({required String game, String saveFolder = '', String saveFile = ''}) async {
  try {
...
Future<String> saveNew({
  ...
  try {
    ...
    if (saveFile.isNotEmpty) {
      ...
      String targetFile;
      if (comment.isEmpty) {
        targetFile = ['SaveLoad', game, profile, strModifiedTimeLast + path.extension(saveFile)].join(Platform.pathSeparator);
      } else {
        targetFile = [
          'SaveLoad',
          game,
          profile,
          '$strModifiedTimeLast@$comment${path.extension(saveFile)}',
